...
import com.cpilosenlaces.microservice.controller.disband.LightningApi;
...
public class LightningApiController implements LightningApi {
    ...
    @Override
    public ResponseEntity<Lightning> getLast1ByDateBetweenAndDisbandId(long minDate, long maxDate, UUID disbandId) {
        return new ResponseEntity<>(ls.findLast1ByDateBetweenAndDisbandId(minDate, maxDate, disbandId), HttpStatus.OK);
    }
    ...
    @Override
    public ResponseEntity<Lightning> getById(UUID id) throws NotFoundException {
        try {
            return new ResponseEntity<>(ls.findById(id), HttpStatus.OK);
        } catch (NotFoundException nfe) {
            throw new NotFoundException("Lightning with ID " + id + " does not exists.");
        }
    }

    @Override
    public ResponseEntity<List<Lightning>> getByDisbandId(UUID disbandId) {
        return new ResponseEntity<>(ls.findByDisbandId(disbandId), HttpStatus.OK);
    }

    @Override
    public ResponseEntity<List<Lightning>> getByDateBetweenAndDisbandId(long minDate, long maxDate,
            UUID disbandId) {
        ...
        return new ResponseEntity<>(ls.findByDateBetweenAndDisbandId(minDate, maxDate, disbandId),
                HttpStatus.OK);
    }

    @Override
    public ResponseEntity<List<Lightning>> getByDateBetween(long minDate, long maxDate) {
        ...
        return new ResponseEntity<>(ls.findByDateBetween(minDate, maxDate), HttpStatus.OK);
    }
    ...
    public ResponseEntity<Lightning> save(LightningDTO lightningDTO) throws NotFoundException, BadRequestException {
        ...
        Lightning lightning = mapper.map(lightningDTO, Lightning.class);
        lightning.setId(UUID.randomUUID());
        lightning.setDate(lightningDTO.getDate());
        lightning.setDisband(disband);

        return new ResponseEntity<>(ls.save(lightning), HttpStatus.CREATED);
    }

    @Override
    public ResponseEntity<List<Lightning>> deleteByDisbandId(UUID disbandId) throws NotFoundException {
        try {
        ...
        List<Lightning> listLightnings = ls.findByDisbandId(disbandId);
        ls.deleteByDisband(listLightnings);

        return new ResponseEntity<>(listLightnings, HttpStatus.OK);
    }
